...
std::vector<std::string> RustGeneratorArray::new_(bool inFunction) {
    ...
    if (inFunction) {
       temp.push_back("let "+this->name+".data = Array::new_infunction(&mut vars, &mut counter,"+std::to_string(this->totalSize)+","+this->name + "->id);");
    } else {
       temp.push_back("let "+this->name+".data = Array::new_outfunc("+std::to_string(this->totalSize)+","+this->name + "->id);");
    }
    return temp;
}
