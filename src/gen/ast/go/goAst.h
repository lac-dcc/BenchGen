 ...
 */
class GoStatementCode : public StatementCode {

   public:
    GoStatementCode(std::shared_ptr<Node> stmt, std::shared_ptr<Node> code) : StatementCode(stmt, code) {
    }
 ...
 */
class GoLambdaCode : public LambdaCode {
   public:
    void gen(ProgrammingLanguageGenerator&) override;

    void print(int indent) override;

    ~GoLambdaCode() = default;
};
 ...
 */
class GoId : public Id {

   public:
    GoId(std::string id) : Id(id) {
    }
 ...
 */
class GoContains : public Contains {
   public:
    void gen(ProgrammingLanguageGenerator&) override;
 ...
 */
class GoLoop : public Loop {

   public:
    GoLoop(std::shared_ptr<Node> code) : Loop(code) {
    }
...
class GoCall : public Call {
   ...
   ~GoCall(){};
    ...
    GoCall(int id, std::shared_ptr<Node> code) : Call(id, code) {}
 ...
 */
class GoSeq : public Seq {

   public:
    GoSeq(std::shared_ptr<Node> code) : Seq(code) {
    }
 ...
 */
class GoIf : public If {
  
   public:
    GoIf(std::shared_ptr<Node> c1, std::shared_ptr<Node> c2) : If(c1, c2) {
    }
